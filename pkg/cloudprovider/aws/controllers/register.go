...
func Register(ctx context.Context, provider *aws.CloudProvider, opts *controllers.ControllerOptions) (ret []controllers.Controller) {
	...
	if opts.Config.EnableInterruptionHandling() {
		...
		infraController := polling.NewController(
			infrastructure.NewReconciler(infraProvider),
			polling.Options{Name: infrastructure.Name, MetricSubsystemName: infrastructure.MetricSubsystemName},
		)
		notificationController := polling.NewController(
			notification.NewReconciler(opts.KubeClient, rec, opts.Cluster, provider.SQSProvider(), provider.InstanceTypeProvider(), infraController),
			polling.Options{Name: notification.Name, MetricSubsystemName: notification.MetricSubsystemName},
		)
